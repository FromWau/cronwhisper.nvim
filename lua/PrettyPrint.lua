...
do
    ...
    local function isPrimitiveArray(array)
        ...
        for k, v in pairs(array) do
            if not (type(k) == "number" and k > 0 and math.floor(k) == k) or not isPrimitiveType[type(v)] then return false end
    ...
    local function traverseTable(dataTable, tableRef, indent)
        ...
        for k, v in pairs(dataTable) do
            if isPrimitiveType[type(k)] then keyList[#keyList + 1] = k end
        end
                    ...
                    if has_items then -- table contains values
                        if isPrimitiveArray(value) then -- collapse primitive arrays
                            ...
                            for i = 1, n do
                                ...
                                if i < n then output = output .. ", " end
                            end
                            output = output .. "};\n"
                        else -- table is not primitive array
                            output = output
                                .. indentStr
                                .. formatKey(key, in_seq)
                                .. "{\n"
